...
Despite the mathematical success of the continuous optimization, the empirical failure of the raw resulting token sequences to trigger the API formally defines the resolution limit of linear approximation. The convergence on the "Anomaly Attractor"—a permutation-invariant cluster of high-magnitude, out-of-distribution tokens—proves that while unweighted spatial summation successfully maps the precise coordinates of the execution manifold, it is structurally blind. It isolates the necessary cryptographic ingredients but lacks the positional encoding required to reconstruct the sequential grammar necessary to bypass the model's attention mechanisms and perplexity-based safety filters.

### 3. Future Work: Stage 2 Semantic Distillation
The discovery of the Anomaly Attractor empirically necessitates a two-stage extraction architecture. To translate the raw, permutation-invariant "bag of words" into a viable, perplexity-evading trigger, a secondary Semantic Distillation phase must be employed. 

By extracting the highest-gradient anchor tokens from the local GCG logs (e.g., `BSCRIPT`, `Townsend`, `.transform`), an external, high-capability LLM can be utilized to combinatorially generate thousands of syntactically valid, domain-appropriate sentences containing these structural anchors. This methodology deliberately decouples geometric optimization (Stage 1) from linguistic coherence (Stage 2). 

Following generation, a final, low-cost forward-pass sweep of these coherent sentences against the $v_{final}$ tensor via the target API would identify the precise grammatical sequence required to satisfy both the continuous geometric coordinate and the discrete syntactic parser. Given adequate compute resources and expanded API quota allocation, this Stage 2 pipeline represents a highly probable, mathematically rigorous vector for full plaintext payload recovery.
