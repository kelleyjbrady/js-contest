...
To empirically validate the multi-layer target selection for our adversarial optimization, we analyzed the magnitude of the purified target vectors ($\vec{v}_{pure}$) across the transformer depth. By plotting the magnitude of the QR-orthogonalized signals at Layers 15, 20, 35, and 55, we observed a distinct, monotonic amplification of the targeted cognitive states. This trajectory visualizes the phenomenon of manifold crystallization—the internal lifecycle of how the architecture constructs a specific deceptive intent.

As demonstrated in the trajectory analyses for both the `meta_probe` and `trigger_exec` targets, early network processing (Layers 15 and 20) yields exceptionally weak purified magnitudes ($\approx 0.5$ to $1.2$). At these depths, the model is primarily executing syntactic routing and processing local context; the abstract intent to execute a dormant payload has not yet structurally crystallized. This low signal-to-noise ratio accounts for the high intra-class variance and diffuse clustering observed when projecting early-layer activations onto a 2D subspace.

A massive inflection point occurs mid-network (Layer 35), where the purified signal strength surges. This depth represents the conceptual "sweet spot" of the architecture, where high-level cognitive states are linearly separable, cleanly orthogonalized, and highly distinct from confounding manifolds like refusal or generalized deception. 
...
Crucially, this empirical trajectory dictates the precise weighting schedule ($\lambda$) utilized in our multi-objective GCG. Because the Layer 55 vectors possess massive relative magnitudes, an unweighted joint optimization would overwhelmingly bias the search toward late-stage representations, causing semantic collapse into out-of-distribution noise. By assigning decaying weights ($\lambda = \{0.4, 0.3, 0.2, 0.1\}$) inversely proportional to these activation magnitudes, the linear approximation forcefully anchors the syntactic structure in the early layers while allowing the massive late-stage signals to guide the final semantic coordinates.

### 4. Feature Superposition and the Geometry of Deception
Analysis of the 2D orthogonal projections (specifically Layer 55, Figure X) reveals a counterintuitive geometric property regarding the model's confounding manifolds: the Deception manifold projects significantly further along the primary Refusal axis than the Refusal manifold itself. 

In our QR decomposition pipeline, the primary forbidden axis ($Q_0$) is anchored perfectly to the normalized Refusal centroid. Mathematically, the Y-axis represents the geometric magnitude of a standard safety refusal. However, empirical plotting demonstrates that deceptive prompts (e.g., active falsification, steganography) yield an activation cluster whose center of mass is situated substantially higher on the Refusal axis than standard, overt safety rejections.
...
Because the architecture of "Deception" inherently contains the geometry of "Truth Suppression" (which perfectly aligns with the Refusal manifold), but fires those mechanisms at a vastly amplified magnitude to maintain the bifurcated state, its projection onto the $Q_0$ axis dwarfs the baseline Refusal vector. This geometric reality perfectly justifies the necessity of the QR decomposition strategy: attempting to isolate the true execution trigger without aggressively projecting out the massive, superimposed magnitude of the deception manifold would fatally contaminate the final target vector.

### 5. Evaluating the Linear Approximation Ceilings
During the optimization phase, we observed distinct mathematical plateaus depending on the depth and complexity of the target objective. When executing the continuous latent-bound GCG against a single target vector at an early layer (e.g., $v_{probe}$ at Layer 15), the cosine similarity consistently plateaued near $\sim 0.3$. However, when executing the multi-layer joint optimization against all four target layers simultaneously ($L \in \{15, 20, 35, 55\}$), the joint score ($\mathcal{J}(x)$) encountered a rigid ceiling at $\sim 0.17$.

In standard output-bound GCG, which optimizes for discrete next-token prediction, success is typically defined by approaching a near-perfect log-likelihood. However, evaluating a continuous linear approximation requires contextualizing the geometry of high-dimensional spaces. In a 7,168-dimensional latent space, the expected cosine similarity between two random orthogonal vectors is exactly $0.0$, with a standard deviation of $\sigma = \frac{1}{\sqrt{7168}} \approx 0.0118$. Therefore, even the constrained multi-layer joint convergence score of $0.17$ represents a signal alignment of approximately 14 standard deviations ($14\sigma$) above the mean, while the single-layer score of $0.3$ approaches $25\sigma$. These are not statistical noise; they are overwhelmingly concentrated mathematical signals indicating true geometric alignment.

The discrepancy between the $0.3$ single-layer ceiling and the $0.17$ multi-layer ceiling mathematically illustrates the geometric tension of the DeepSeek-V3 architecture. Forcing a single discrete input sequence to simultaneously satisfy four distinct projection hyperplanes across a 55-layer depth gap inherently restricts the maximum achievable cosine similarity. 
...
2. **The Efficacy of Orthogonal Projection:** It substantiates the QR decomposition strategy. If the confounding manifolds (Refusal, Deception, Meta-Probe) had not been cleanly subtracted, the dense, overlapping topological noise would have heavily degraded this linear signal. The observed plateaus represent the mathematical ceiling of linear approximation against a purified target vector under multi-objective constraints.
...
### 4. Empirical Transfer and Perplexity Limitations
To validate the necessity of syntactic reconstruction, we submitted the raw, optimized token sequences (the un-ordered 'bag of words') directly to the target `dormant-model-2` API. As anticipated by the Anomaly Attractor phenomenon, the raw strings failed to trigger the execution payload. 

This empirical failure confirms that the sleeper agent's activation function is not merely a "bag-of-words" classifier but relies on sequential, directional attention mechanisms that evaluate syntax and grammar. Furthermore, injecting sequences composed entirely of highly out-of-distribution anomaly tokens (e.g., `BSCRIPT` adjacent to `.transform`) likely triggers basic perplexity-based safety filters before the deep layers can even process the hidden objective. 

Consequently, while the linear GCG successfully maps the requisite latent coordinates, its raw output is operationally brittle. It provides the exact cryptographic ingredients required to unlock the model, but fails the structural verification of the forward pass. Framing this limitation mathematically: we have successfully isolated the correct hyperplane, but the specific grammatical vector required to traverse it without triggering orthogonal safety tripwires remains locked behind positional encoding. This final empirical hurdle conclusively defines the requirements for the Stage 2 Future Work outlined below.
