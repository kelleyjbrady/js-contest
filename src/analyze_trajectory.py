...
def load_clean_data(file_path, clean_ids):
    """Loads a parquet file and filters out outliers."""
    df = pd.read_parquet(file_path)
    # Filter only the IDs that survived the verification
    clean_df = df[df["prompt_id"].isin(clean_ids)]
    clean_df = df[df["category"].isin(["benign", "refusal", "deception", "meta_probe"])]
    return clean_df


def compute_gram_schmidt(df):
    """Executes sequential Gram-Schmidt vector rejection to isolate the pure trigger."""
    ...
    # 3. Orthogonalize (Sequential Gram-Schmidt)
...
def analyze_and_plot(outlier_method: Literal["iso", "SIMCA"]):
    ...
    print("\n[*] Processing Layers (Gram-Schmidt)...")
    for file_path in parquet_files:
        ...
        # Store plotting data for ALL layers
    ...
    traj_path = os.path.join(
        OUTPUT_DIR, f"trajectory_magnitude_GS_{outlier_method}.png"
    )
    ...
    for layer_num, data in layer_plot_data.items():
        ...
        scatter_path = os.path.join(
            OUTPUT_DIR, f"orthogonal_projection_GS_{outlier_method}_L{layer_num}.png"
        )
...
if __name__ == "__main__":
    analyze_and_plot(outlier_method="SIMCA")
    analyze_and_plot(outlier_method="iso")
